...
class RuntimeConfigurationTest extends TestCase
{
    public function test_env_example_uses_havana_runtime_defaults(): void
    {
        ...
        $this->assertSame('/opt/git/Havana', $env['HAVANA_BASE_PATH']);
        $this->assertSame('"${HAVANA_BASE_PATH}/tools/www-tpl"', $env['HAVANA_TEMPLATE_PATH']);
        $this->assertSame('default', $env['HAVANA_TEMPLATE_NAME']);
        $this->assertSame('locale-en.ini', $env['HAVANA_LOCALE_FILE']);
        $this->assertSame('"${HAVANA_BASE_PATH}/tools/www"', $env['HAVANA_PUBLIC_PATH']);
    ...
    public function test_runtime_config_resolves_havana_env_values(): void
    {
        config([
            ...
            'queue.default' => 'sync',
            'mail.default' => 'log',
            'havana.base_path' => '/opt/git/Havana',
            'havana.template_path' => '/opt/git/Havana/tools/www-tpl',
            'havana.template_name' => 'default',
            'havana.locale_file' => 'locale-en.ini',
            'havana.public_path' => '/opt/git/Havana/tools/www',
        ...
        $this->assertSame('/opt/git/Havana', config('havana.base_path'));
        $this->assertSame('/opt/git/Havana/tools/www-tpl', config('havana.template_path'));
        $this->assertSame('default', config('havana.template_name'));
        $this->assertSame('locale-en.ini', config('havana.locale_file'));
        $this->assertSame('/opt/git/Havana/tools/www', config('havana.public_path'));
